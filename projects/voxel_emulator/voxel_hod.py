        ...
        for phase in range(start_phase, start_phase + n_phase):
            ...
            for hod in range(start_hod, start_hod + n_hod):
                ...
                if args.save_mocks:
                    ...
                    output_fn = Path(
                        output_dir,
                        f'AbacusSummit_AP_base_c{cosmo:03}_ph{phase:03}_hod{hod}.npy'
                    )
                ...
                for los in ['x', 'y', 'z']:
                    ...
                    if args.save_voids:
                        ...
                        output_dir = Path(f'/pscratch/sd/t/tsfraser/voxel_emulator/voxel_voids/HOD/{dataset}/',
                            f'AbacusSummit_base_c{cosmo:03}_ph{phase:03}/z0.500/')
                        Path(output_dir).mkdir(parents=True, exist_ok=True)
                        output_fn = Path(
                            output_dir,
...
#                                           q_perp=q_perp, q_para=q_para)
                    ...
                    output_dir = Path(f'/pscratch/sd/t/tsfraser/voxel_emulator/voxel_multipoles/HOD/{dataset}/',
                        f'AbacusSummit_base_c{cosmo:03}_ph{phase:03}/z0.500/')
                    Path(output_dir).mkdir(parents=True, exist_ok=True)
                    output_fn = Path(
                        output_dir,

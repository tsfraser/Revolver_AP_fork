...
if __name__ == '__main__':
    ...
    for cosmo in range(start_cosmo, start_cosmo + n_cosmo):
        ...
        #new_q_perp,new_q_para = discretize_AP(cellsize/q,boxsize,q_perp,q_para,los)
        ...
        for phase in range(start_phase, start_phase + n_phase):
            ...
            for hod in range(start_hod, start_hod + n_hod):
                ...
                if args.save_mocks:
                    output_dir = Path(f'/pscratch/sd/t/tsfraser/voxel_emulator/HOD/{dataset}/small/',
                ...
                for los in ['x', 'y', 'z']:
                    ...
                    handle = f'/pscratch/sd/t/tsfraser/tmp/voxel_c{cosmo:03}_ph{phase:03}_hod{hod}_los{los}'
                    print("WHERE IS THIS")
                    ...
                    if args.save_voids:
                        ...
                        output_dir = Path(f'/pscratch/sd/t/tsfraser/voxel_emulator/voxel_voids/HOD/{dataset}/small/',
                            f'AbacusSummit_small_c{cosmo:03}_ph{phase:03}/z0.500/')
                        Path(output_dir).mkdir(parents=True, exist_ok=True)
                        output_fn = Path(
                            output_dir,
                            f'voxel_voids_AP_Rs{smoothing_radius}_c{cosmo:03}_ph{phase:03}_hod{hod}_los{los}.npy'
                        )
                    ...
                    output_dir = Path(f'/pscratch/sd/t/tsfraser/voxel_emulator/voxel_multipoles/HOD/{dataset}/small/',
                        f'AbacusSummit_small_c{cosmo:03}_ph{phase:03}/z0.500/')
                    Path(output_dir).mkdir(parents=True, exist_ok=True)
                    output_fn = Path(
                        output_dir,
                        f'voxel_multipoles_AP_Rs{smoothing_radius}_c{cosmo:03}_ph{phase:03}_hod{hod}_los{los}.npy'
                    )
